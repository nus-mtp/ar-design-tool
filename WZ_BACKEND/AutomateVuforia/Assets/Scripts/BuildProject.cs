...
public class BuildProject : MonoBehaviour {
    ...
    [MenuItem("File/Setup Objects")]
    public static void SetUpModels(GameObject marker)
    {
        string[] fileNames = Directory.GetFiles(Application.dataPath + MODELS_DATA_PATH);
       
        List<string> modelNames = new List<string>();
      
        foreach(string fileName in fileNames)
        {
            if( acceptedModelFormats.Contains(Path.GetExtension(fileName))){
    ...
    [MenuItem("File/Read Files")]
    private static void ReadWrapper()
    {
        ReadDataSetTrackerName(MarkerType.IMAGE);
    }
